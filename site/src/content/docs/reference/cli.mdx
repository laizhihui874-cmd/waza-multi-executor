...
| `--executor` | | string | `config.executor` | Override the YAML executor for this run |
...
| `--suggest` | | bool | false | Generate a Copilot suggestion report based on test outcomes; requires `copilot-sdk` |
...
| `--engine` | `copilot-sdk` | `copilot-sdk`, `codex-cli`, `claude-cli`, `hermes-cli`, or `generic-cli`. |
| `--skill` | _(none)_ | Skill name to evaluate. Required. |
