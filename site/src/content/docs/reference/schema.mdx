...
**Type:** string
**Default:** `copilot-sdk`
**Options:** `copilot-sdk`, `codex-cli`, `claude-cli`, `hermes-cli`, `generic-cli`

Execution engine:

- `copilot-sdk` — Copilot SDK sessions with full event evidence
- `codex-cli` — Local Codex JSONL execution
- `claude-cli` — Local Claude stream-JSON execution
- `hermes-cli` — Local Hermes one-shot execution
- `generic-cli` — User-defined argv protocol (`text` or `jsonl`)
...
Configures a CLI-backed executor. Waza passes `command` and `args` directly to the operating system without a shell. `generic-cli` requires `command`; built-in CLI executors use `codex`, `claude`, or `hermes` by default.
...
The generic JSONL protocol accepts `assistant_message`, `result`, `usage`, and `error` events. Placeholders such as `{workspace}`, `{workdir}`, `{skill_dir}`, and `{model}` are expanded per run. `{prompt}` is only expanded when `prompt_transport: argument` is explicit.
...
Copilot compatibility option controlling whether Waza injects the full target `SKILL.md` or `.agent.md` body. CLI executors use native skill discovery and ignore this field with a migration warning.
